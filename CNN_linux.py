...
if __name__ == '__main__':
    ...
    for epoch in range(n_epoch):
        ...
        for x_train_a, y_train_a in minibatches(x_train, y_train, batch_size, shuffle=True):
            _, err, ac,summary = sess.run([train_op, loss, acc,merged_summary_op], feed_dict={x: x_train_a, y_: y_train_a})
            train_loss += err;
            train_acc += ac;
            n_batch += 1
        ...
        total_batch = int(len(x_val) / batch_size)
        for x_val_a, y_val_a in minibatches(x_val, y_val, batch_size, shuffle=False):
            err, ac, result,summary = sess.run([loss, acc,train_result,merged_summary_op], feed_dict={x: x_val_a, y_: y_val_a})
            val_loss += err;
            val_acc += ac;
            n_batch += 1
