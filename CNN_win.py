...
if __name__ == '__main__':
    ...
    np.save(r'C:\Users\wangbc1\Desktop\OCR\data\data.npy',data)
    np.save(r'C:\Users\wangbc1\Desktop\OCR\data\label.npy', label)
    # data = np.load(r'C:\Users\wangbc1\Desktop\OCR\data\data.npy')
    # label = np.load(r'C:\Users\wangbc1\Desktop\OCR\data\label.npy')
    ...
    tf.add_to_collection('pred_network', train_result)
    max_acc = 0
    train_loss_list = []
    train_acc_list = []
    val_loss_list = []
    val_acc_list = []
    noise_factor_list = []
    f = open(r'C:\Users\wangbc1\Desktop\OCR\model\detection\22\准确率纪录.txt','w')
    ...
    for epoch in range(n_epoch):
        ...
        print("   noise factor %d: %f" % (epoch + 1, (nf / (batch_size * n_batch))))
        print('\n')
        ...
        f.write("   noise factor %d: %f\n\n" % (epoch + 1, (nf / (128 * n_batch))))
        val_loss_list.append(val_loss / n_batch)
        val_acc_list.append(val_acc / n_batch)
        noise_factor_list.append(nf / (batch_size * n_batch))

        if val_acc / n_batch > max_acc:
            max_acc = val_acc / n_batch
            saver.save(sess, r'C:\Users\wangbc1\Desktop\OCR\model\detection\22\CNN-Model', global_step=epoch + 1)
    sess.close()
    f.close()
