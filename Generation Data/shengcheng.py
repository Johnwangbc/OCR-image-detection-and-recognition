...
def main(db_fname):
    ...
    num = 1
    with open(r'E:\OCRImage\shibieziliao\label.txt','w') as f:
        for k in dsets:
            sign = 0
            rgb = db['data'][k][...]
            ...
            for i in range(len(txt)):
                ...
                for j in range(len(newtxt)):
                    ...
                    if min(bb[1,:])<0 or max(bb[1,:])<0 or min(bb[0,:])<0 or max(bb[0,:])<0:
                        ...
                        cv2.imwrite(r'E:\OCRImage\shibieziliao\%d.jpg' % num, temp)
                        f.write('%d.jpg ' % num + newtxt[j].encode('utf-8') + ' \n')
                        num += 1
        db.close()

if __name__=='__main__':
    main(r'C:\Users\wangbc1\Desktop\SynthText_Chinese_version-master\SynthText_Chinese_version-master\results\SynthText_8000.h5')
