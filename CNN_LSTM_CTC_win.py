...
path = r'E:\OCRImage\image\image'
w = 100
h = 32
c = 3
batch_size =128
layer_params = [ [  64, 3, 'same',  'conv1', True],
                 [ 128, 3, 'same',  'conv2', True], # pool
                 [ 256, 3, 'same',  'conv3', True],
                 [ 256, 3, 'same',  'conv4', True], # hpool
                 [ 512, 3, 'same',  'conv5', True],
                 [ 512, 3, 'same',  'conv6', True], # hpool
                 [ 512, 2, 'valid',  'conv7', True]] #
...
def read_img(path):
    with open(path+'\\label.txt','r') as f:
        ...
        for i in range(len(a)):
            img = cv2.imread(path+'\\'+a[i].split()[0])
            try:
            ...
            for j in range(15):
                if j<=len(b)-1:
                    ...
                    temp.append(-1)
            imgs.append(img)
            seqlengths.append(len(b))
            labels.append(temp)
            temp = []
            print (num)
            num+=1
    return np.asarray(imgs,np.float32),np.asarray(labels,np.int32),np.asarray(seqlengths,np.int32)
...
# def lstm_cell():
#     lstm = rnn.BasicLSTMCell(256, forget_bias=1.0, state_is_tuple=True)
#     return lstm
...
if __name__ == '__main__':
    # [data, label,seqlength] = read_img(path)
    # np.save('/home/wangbc1/OCR/data/data2.npy',data)
    # np.save('/home/wangbc1/OCR/data/label2.npy', label)
    # np.save('/home/wangbc1/OCR/data/seqlength2.npy', seqlength)
    print ('Loading Image ......')
    data = np.load(r'C:\Users\wangbc1\Desktop\OCR\data\data2.npy')
    label = np.load(r'C:\Users\wangbc1\Desktop\OCR\data\label2.npy')
    seqlength = np.load(r'C:\Users\wangbc1\Desktop\OCR\data\seqlength2.npy')
    ...
    # -----------------构建网络----------------------
    # 占位符
    x = tf.placeholder(tf.float32, shape=[batch_size, h, w, c], name='x')
    y_ = tf.sparse_placeholder(tf.int32,name='y_')
    seq_length = tf.placeholder(tf.int32, shape=[batch_size, ], name='seq_length')
    ...
    train_op = tf.train.AdamOptimizer(0.01,name = 'train_op').apply_gradients(zip(grads, var_trainable_op))
    # train_op = tf.train.AdamOptimizer(0.01,name = 'train_op').minimize(loss_train)
    ...
    tf.add_to_collection('pred_network', dense1)
    max_acc = 0
    f = open(r'C:\Users\wangbc1\Desktop\OCR\model\recognition\record.txt','w')
    summary_writer = tf.summary.FileWriter(r'C:\Users\wangbc1\Desktop\OCR\model',
    ...
    for epoch in range(n_epoch):
        ...
        shapes = np.array([batch_size, 15], dtype=np.int32)
        ...
        if 1-(val_err / n_batch) / n_batch > max_acc:
            max_acc = 1-(val_err / n_batch) / n_batch
            saver.save(sess, r'C:\Users\wangbc1\Desktop\OCR\model\recognition\CNN+LSTM+CTC-Model', global_step=epoch + 1)
    sess.close()
    f.close()
